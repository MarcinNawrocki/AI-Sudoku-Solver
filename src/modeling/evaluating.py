...
def solve_batch(np_boards, np_solved_boards, model):
    ...
    for i in np.ndindex(np_boards.shape[:1]):
        ...
        np_results[i] = compare_sudoku(
            solved, (np_solved_boards[i]+1).reshape(9, 9))
...
def solve_human_approach(np_sample_board, model):
    ...
    while(1):
        ...
        predictions_int = np.argmax(predictions, axis=1).reshape(
            (9, 9))+1  # change range of digits from <0,8> to <1,9>
        propabilities = np.around(
            np.max(predictions, axis=1).reshape((9, 9)), 2)
        ...
        blanks_indices = np.array(
            [(i, j) for i, j in zip(blanks_indices[0], blanks_indices[1])])
        position_to_fill = blanks_indices[propabilities[zeros].argmax()]
        # fill position with the highest probability
        value_to_fill = predictions_int[position_to_fill[0],
                                        position_to_fill[1]]
